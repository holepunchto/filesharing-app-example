  ...
  for await (const file of files) {
    ...
    const $file = $(`
      <div class="file">
        ...
        <span class="file__download-status file__download-status--hidden">
          <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" style="margin: auto; background: rgb(255, 255, 255); display: inline; shape-rendering: auto;" width="15px" height="15px" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid">
    ...
    $('.file__delete', $file).on('click', () => drive.del(file.key))
    if (!allowDeletion) $('.file__delete', $file).remove()

    $('.file__name', $file).on('click', async () => {
      $('.file__download-status', $file).removeClass('file__download-status--hidden')
      const rs = drive.createReadStream(file.key, {
        timeout: 5000 // is this timeout untit it starts, or until it ends?
      })
      const ws = userLocalDrive.createWriteStream(filename)
      rs.pipe(ws)
      rs.on('end', () => {
        $('.file__download-status', $file).addClass('file__download-status--hidden')
      ...
      rs.on('error', err => {
        ...
        if (hasTimedOut) {
          alert(`Could not start download of ${filename}. Maybe the network does not have the file.`)
        } else {
          alert(err.message)
        }
      })
